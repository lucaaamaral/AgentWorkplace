...
//! connection), with the CX-2/5/8/11 behaviors: busy threads delay turn/start,
//! unloaded threads are resumed first, an unreachable app-server holds the
//! delivery while the session is present, and approval requests are never
//! answered by the adapter.
...
struct FakeBehavior {
    /// thread/read reports an active (busy) thread this many times before
    /// going idle.
    busy_reads: usize,
                ...
                while let Some(Ok(msg)) = source.next().await {
                    ...
                    let result = match method {
                        ...
                        "thread/read" => {
                            ...
                            let busy = reads_seen < behavior.busy_reads && !turn_started;
                            reads_seen += 1;
                            let status = if busy { "active" } else { "idle" };
                            let turns: Vec<Value> = if turn_started {
...
async fn codex_registered_principal_is_delivered_via_attach() {
    let (app_server, _stats) = fake_app_server(FakeBehavior::default()).await;
...
#[tokio::test]
async fn busy_thread_delays_turn_start_until_idle() {
    // The thread reports busy for the first three reads: the adapter must
    // hold turn/start (CX-2 — firing while busy silently drops the message).
    ...
    .await
    .expect("delivery through a briefly-busy thread should still process");
