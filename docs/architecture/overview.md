...
- **Delivery queue.** Per-principal outbound state with acknowledgment status inspectable by the human: a message to a busy session (or one whose delivery path is transiently unavailable) is held until deliverable; a message to a disconnected principal fails per-recipient — no store-and-forward across sessions. Once handed to a harness, the adapter simply relays — ordering and queuing follow the harness's own message flow, which already implements them; adapters carry no retention or expiry logic of their own.
...
- Session closed: delivery to that recipient fails (per-recipient `failed`, visible to the human; the sender's delivery report states it). The returning agent catches up via history.
